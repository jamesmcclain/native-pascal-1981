#!/usr/bin/env bash
# Builds units_basic.pas (a host PROGRAM) linked against a separately
# compiled UNIT implementation (units_basic.mathutil.impl), demonstrating
...
"$bin_dir/lexer" < units_basic.mathutil.impl | "$bin_dir/parser" | "$bin_dir/typechecker" | "$bin_dir/codegen" > "$work_dir/mathutil.ll"
"$clang_bin" -O1 -c "$work_dir/mathutil.ll" -o "$work_dir/mathutil.o"

"$bin_dir/lexer" < units_basic.pas | "$bin_dir/parser" | "$bin_dir/typechecker" | "$bin_dir/codegen" > "$work_dir/host.ll"
